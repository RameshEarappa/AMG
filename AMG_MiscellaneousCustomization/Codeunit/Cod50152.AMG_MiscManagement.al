codeunit 50152 AMG_MiscManagement
{
    ...
    procedure CheckDimensions(ParTableId: Integer;
    ParNo: Code[20])
    var
    ...
    begin
        ...
        IF Not (DefaultDimension.GET(ParTableId, ParNo, GLSetup."Shortcut Dimension 3 Code")) Then
            ...
            IF Not (DefaultDimension."Value Posting" in [DefaultDimension."Value Posting"::"Code Mandatory", DefaultDimension."Value Posting"::"Same Code"]) Then DefaultDimension.FieldError("Value Posting", 'Code Mandatory or same code');
        end
    end;
    // Purchase Requisition. //
    procedure CreatePurchDocument(Var ParPurchReqHeader: Record AMG_PurchRequisitionHeader;
    Var ParPurchReqLine: Record AMG_PurchRequisitionLine;
    Var ParPurchType: Option Quote,"Order")
    var
    ...
    begin
        ...
        PurchaseHeaderRec.Backcharge := ParPurchReqHeader.Backcharge;
        PurchaseHeaderRec."Backcharge To" := ParPurchReqHeader."Backcharge To";
    ...
    begin
        if ParReqLine.FindSet() then
            repeat
                ...
                if ParReqLine.OrderCreated then ParReqLine.FieldError(OrderCreated);
    ...
    begin
        if ParReqLine.FindSet() then
            repeat
                ...
                if ParReqLine.QuoteCreated then ParReqLine.FieldError(QuoteCreated);
            until ParReqLine.Next() = 0;
    end;

    procedure CheckQuoteExistForVendor(Var ParReqLine: Record AMG_PurchRequisitionLine;
    var ParVendRec: Record Vendor)
    var
        ...
        if ParReqLine.FindSet() then
            repeat
                ...
                if ParReqLine.QuoteCreated then begin
                    ...
                    if PurchLine.FindFirst() then Error(ErrorQuoteAlreadyExist, ParReqLine."Document No.", ParVendRec."No.", Format(ParReqLine.Type), ParReqLine."No.");
                end;
            until ParReqLine.Next() = 0;
    end;
    ...
    begin
        ...
        Clear(RemTotalCost);
        JobSetup.Get();
        IF (JobSetup.AMG_NotAllowExceedBudget) And (ParRec."Job Planning Line No." > 0) And ((ParRec."Job No." <> '') AND (ParRec."Job Task No." <> '')) Then Begin
            ...
            IF ParRec."Job Planning Line No." > 0 Then JobPlanningLine.SetRange("Line No.", ParRec."Job Planning Line No.");
            IF ParRec.Type = ParRec.Type::"G/L Account" Then JobPlanningLine.SetRange(Type, JobPlanningLine.Type::"G/L Account");
            IF ParRec.Type = ParRec.Type::Item Then JobPlanningLine.SetRange(Type, JobPlanningLine.Type::Item);
            IF ParRec.Type = ParRec.Type::Resource Then JobPlanningLine.SetRange(Type, JobPlanningLine.Type::Resource);
            ...
            IF JobPlanningLine.FindSet() then Begin
                ...
                IF ParRec.Type = ParRec.Type::"G/L Account" Then JobPlanningLine.SetRange(Type, JobLedgerEntries.Type::"G/L Account");
                IF ParRec.Type = ParRec.Type::Item Then JobPlanningLine.SetRange(Type, JobLedgerEntries.Type::Item);
                IF ParRec.Type = ParRec.Type::Resource Then JobPlanningLine.SetRange(Type, JobLedgerEntries.Type::Resource);
                JobLedgerEntries.SetRange("No.", ParRec."No.");
                IF ParRec."Job Planning Line No." > 0 Then JobLedgerEntries.SetRange(AMG_JobPlanningLineNo, ParRec."Job Planning Line No.");
                ...
                IF RemTotalCost < 0 then RemTotalCost := 0;
                IF RemTotalAmount < 0 then RemTotalAmount := 0;
            end
            Else
            ...
            IF (RemQty < ParRec.Quantity) then Begin //AND (Not (ParRec.Type = ParRec.Type::"G/L Account")) THEN Begin
                IF RemQty = 0 then Error(ErrorNoRemainingUsage);
            ...
            IF ParRec."Account Type" = ParRec."Account Type"::"G/L Account" Then begin
                ...
                IF JobPlanningLine.FindSet() then Begin
                    ...
                    IF RemTotalCost < ParRec."Amount (LCY)" THEN begin
                        IF RemTotalCost = 0 then Error(ErrorNoRemainingUsage);
                        BugetLimitErrorTxt := StrSubstNo('must not be greater than remaining budget: %1', FORMAT(RemTotalCost));
        ...
        IF (JobSetup.AMG_NotAllowExceedBudget) And (ParRec."Job Planning Line No." > 0) And ((ParRec."Job No." <> '') AND (ParRec."Job Task No." <> '')) Then Begin
            IF ParRec.Type in [ParRec.Type::"G/L Account", ParRec.Type::Item] Then begin
                ...
                IF JobPlanningLine.FindSet() then Begin
                    ...
                    JobLedgerEntries.SetRange("Job No.", ParRec."Job No.");
                    JobLedgerEntries.SetRange("Job Task No.", ParRec."Job Task No.");
                    IF ParRec.Type = ParRec.Type::"G/L Account" Then JobLedgerEntries.SetRange(Type, JobLedgerEntries.Type::"G/L Account");
                    IF ParRec.Type = ParRec.Type::Item Then JobLedgerEntries.SetRange(Type, JobLedgerEntries.Type::item);
                    JobLedgerEntries.SetRange("No.", ParRec."No.");
                    IF ParRec."Job Planning Line No." > 0 Then JobLedgerEntries.SetRange(AMG_JobPlanningLineNo, ParRec."Job Planning Line No.");
                    IF JobLedgerEntries.FindSet() then
                        repeat
    ...
    procedure CopyGLAccount(var ParGLAccount: Record "G/L Account";
    ParRec: Record "G/L Account")
    var
    begin
        ...
        ParGLAccount.SystemId := ParRec.SystemId;
